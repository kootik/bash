...
alias rm='rm -i'                # Запрашивает подтверждение перед удалением каждого файла, предотвращая случайные потери.
alias cp='cp -i'                # Запрашивает подтверждение перед перезаписью существующего файла.
alias mv='mv -i'                # Запрашивает подтверждение перед перезаписью файла при перемещении.
