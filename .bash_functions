...
# УЛУЧШЕНО: Более надежная проверка успеха, основанная на повторной проверке процессов.
killg() {
    if [ -z "$1" ]; then
