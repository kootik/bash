...
du() {
  local path=${1:-.}          # Путь: первый аргумент или текущая папка (.), если он пуст
  local depth=${2:-2}         # Глубина: второй аргумент или 2, если он пуст
  local count=${3:-20}        # Кол-во строк: третий аргумент или 20, если он пуст

  echo "🔍 Поиск в '$path' на глубину '$depth' (топ $count):"
  du -h --max-depth="$depth" "$path" | sort -rh | head -n "$count"
}
