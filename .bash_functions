...
# killg - "Умное" завершение процесса по имени.
# УЛУЧШЕНО: Сначала пытается завершить процесс от текущего пользователя.
# Если не удается, автоматически пробует с правами sudo.
killg() {
    if [ -z "$1" ]; then
    ...
    # Проверяем, существуют ли такие процессы
